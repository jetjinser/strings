...
class ImageProcessing:
    ...
    def process(self):
        ...
        # 创建小图标的框架
        framework = Image.open('framework.png').convert('L')
        ...
        # 指定文字, 文字的字体和位置
        txt = self._txt.split('\n')
        font = ImageFont.truetype(r'REEJI-HonghuangLiGB-SemiBold-2.ttf', 25)
        count = 0
        for i in txt:
            text_size = font.getsize(i)
            where = ((self._image.size[0]-text_size[0])/2, (self._image.size[1]-text_size[1])/2 + 125 + 30 * count)
        ...
        with open('./data/tips.json', 'r', encoding='utf-8') as file:
            ...
            romdom = randint(0, int(num))
            writer = data['{}'.format(romdom)]

        # 写入tips
        draw = ImageDraw.Draw(target)
        font = ImageFont.truetype(r'REEJI-HonghuangLiGB-SemiBold-2.ttf', 20)
    ...
    def save(self):
        ...
        保存图片
        """
        self.process().save(self._name + '.png')

if __name__ == '__main__':
    image = '../venv42.jpg'
    image = ImageProcessing(image, '签到成功\n coin +5', 256, 'demo')
    image.save()
